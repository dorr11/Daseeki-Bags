...
function Owners:OnLoad()
	...
	for i, realm in ipairs(Addon.IsRetail and GetKeysArray(DaseekiBagsAccount) or self.realms) do
		for id, cache in pairs(realm ~= 'account' and DaseekiBagsAccount[realm] or Addon.None) do
			self:New(id, realm)
		end
	end
...
function Owners:Sort()
	for i, owner in ipairs(self.ordered) do
		owner.offline = owner ~= Addon.player
	end
...
function Owners:New(id, realm)
	local cache = GetOrCreateTableEntry(GetOrCreateTableEntry(DaseekiBagsAccount, realm), id)
...
function Owners:Delete()
	...
	DaseekiBagsAccount[self.realm][self.id] = nil
