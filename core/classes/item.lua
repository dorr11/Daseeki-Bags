...
Item.Backgrounds = {
	...
	-- [3] reserved for Modern (retail)
	[4] = 'Interface/Icons/INV_Misc_Head_Lion_01',
	[5] = function(icon) icon:SetAtlas('MountJournalIcons-Alliance') end,
	[6] = function(icon) icon:SetAtlas('MountJournalIcons-Horde') end,
}
...
function Item:Construct()
	...
	local bf = CreateFrame('Frame', nil, b, 'BackdropTemplate')
	bf:SetAllPoints()
	bf:EnableMouse(false)
	bf:SetFrameLevel(b:GetFrameLevel() + 5)
	bf:SetBackdrop({edgeFile = 'Interface/Buttons/WHITE8X8', edgeSize = 2, insets = {left=0, right=0, top=0, bottom=0}})
	bf:SetBackdropBorderColor(1, 1, 1, 0)
	b.SlotBorder = bf
...
function Item:Update(info)
	...
	local bg = self.Backgrounds[Addon.sets.slotBackground]
	if type(bg) == 'function' then
		if not self.hasItem then
			self.icon:SetTexture(nil)
			bg(self.icon)
		else
			SetItemButtonTexture(self, self.info.iconFileID)
		end
	else
		SetItemButtonTexture(self, self.hasItem and self.info.iconFileID or bg)
	end
	self.icon:SetAlpha(self.hasItem and 1 or (Addon.sets.slotAlpha or 1))
	SetItemButtonCount(self, self.info.stackCount)

	self.icon:SetVertexColor(1, 1, 1)
	if self.SlotBorder then
		local bc = Addon.sets.slotBorderColor or {1, 1, 1, 0}
		self.SlotBorder:SetBackdropBorderColor(bc[1], bc[2], bc[3], bc[4] or 0)
	end
end
